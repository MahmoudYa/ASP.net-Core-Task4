...
public class ModuleModel
{
    ...
    public ModuleModel(String model, String controller, String? area)
    {
        ...
        Type modelType = assembly.GetType($"REAL_Estate.Objects.{model}") ?? typeof(AModel);
        Type viewType = assembly.GetType($"REAL_Estate.Objects.{model}View") ?? typeof(AView<>);
        BindingFlags declaredOnly = BindingFlags.Public | BindingFlags.Instance | BindingFlags.DeclaredOnly;

        ControllerNamespace = $"REAL_Estate.Controllers.{area}".Trim('.');
        ...
        {
            Type type = assembly.GetType($"REAL_Estate.Objects.{model}{view}View") ?? viewType;
